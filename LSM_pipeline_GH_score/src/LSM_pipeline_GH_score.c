...
static SHARED_SRAM uint32_t session_bout_index[MAX_SESSION_STRIDES];
static SHARED_SRAM bool session_turning_step[MAX_SESSION_STRIDES];
static SHARED_SRAM bool session_outlier[MAX_SESSION_STRIDES];
                                           ...
                                           uint32_t *out_stride_count)
{
    ...
    for (uint32_t b = 0u; b < n_bouts; b++) {
        ...
        for (uint32_t i = 0; i < n_strides; i++) {
            ...
            session_bout_index[fv_count] = b;
            session_turning_step[fv_count] = false;
            session_outlier[fv_count] = sim_strides[i].is_outlier;
                                            ...
                                            uint32_t *out_stride_count)
{
    ...
    for (uint32_t b=0; b<n_bouts; b++) {
        ...
        for(uint32_t i=0;i<ns && fv<MAX_SESSION_STRIDES;i++){
            ...
            /* Health-score metadata: keep the live path identical to the
             * simulation path and to the Python cleaned-data model. */
            session_bout_index[fv] = b;
            session_turning_step[fv] = false;
            session_outlier[fv] = live_strides[i].is_outlier;
...
int main(void) {
    ...
    memset(session_bout_index, 0, sizeof(session_bout_index));
    memset(session_turning_step, 0, sizeof(session_turning_step));
    memset(session_outlier, 0, sizeof(session_outlier));
...
#define DEMO_TOTAL_SAMPLES   (LSM_SESSION_SECONDS * (uint32_t)SAMPLE_RATE)
        ...
        for (uint32_t g = 0; g < n_grabbed && total_samples < DEMO_TOTAL_SAMPLES; g++) {
            ...
            if (false && cb_count >= MAX_SAMPLES &&
                ...
                for (uint32_t i = 0; i < n_strides; i++) {
                    ...
                    session_strides[fv_count].stride_idx = fv_count;
                    session_bout_index[fv_count] = minute_number;
                    session_turning_step[fv_count] = false;
                    session_outlier[fv_count] = strides[i].is_outlier;
    ...
    csv_log_session_stats(total_duration_s, total_samples, pipeline_time_sec,
        ...
        gait_score_result_t health_score;  // Exact Python-parity scorer
        gait_health_score_compute_ex(temp_params,
                                     spat_params,
                                     session_outlier,
                                     session_bout_index,
                                     session_turning_step,
                                     fv_count,
                                     "walking",
                                     "left",
                                     &health_score);
